...
int main() {
	...
	printf("Which Convolution would you like to perform?\n");
	printf("1.Convolution with first order derivative of Gaussian Mask\n2.Convolution with second order derivative of Gaussian Mask\n");
	#if DIFF_ARRAYS
	...
	printf("Enter the Length of the Mask( >= 3 && length is odd)\n");
	do {
	scanf("%d", &length_mask);
		if(length_mask < 3)
				printf("Enter a Value greater than or equal to 3\n");
		if(length_mask >= 25) {
				printf("Enter a value lesser than length of the sample array\n");
		}				
	} while(length_mask < 3 || length_mask >= 25);
